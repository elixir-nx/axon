defmodule Axon.LossesTest do
  ...
  describe "ctcloss" do
    test "value for basic case" do
      y_true = Nx.tensor([[0, 2, 0, 3, 0, 4, 0, 1, 0, 0, 0, 0, 0], [0, 2, 0, 2, 0, 4, 0, 1, 0, 0, 0, 0, 0]])
      y_pred = Nx.broadcast(-1.6094379425048828, {2, 10, 5})

      assert Axon.Losses.ctcloss(y_true, y_pred) == Nx.tensor([8.08701229095459, 8.934309959411621])
    end

    test "padding blanks doesn't contribute" do
      y_true1 = Nx.tensor([[0, 2, 0, 3, 0, 4, 0, 1, 0, 0, 0, 0, 0], [0, 2, 0, 2, 0, 4, 0, 1, 0, 0, 0, 0, 0]])
